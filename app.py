...
# --- 2. اتصال Firebase (النظام المضمون) ---
@st.cache_resource
def init_db():
    if not firebase_admin._apps:
        try:
            # هنا بنقرأ بيانات الـ JSON من الـ Secrets اللي هنحطها في موقع Streamlit
            key_dict = st.secrets["textkey"]
            cred = credentials.Certificate(key_dict)
            firebase_admin.initialize_app(cred)
        except Exception as e:
            st.error("خطأ: تأكد من إضافة بيانات key.json في الـ Secrets على موقع Streamlit")
            st.stop()
...
if not st.session_state.opened:
    ...
    if st.button("إرسال التهنئة ❤️", use_container_width=True):
        if name and msg:
            try:
                ...
                st.error("فشل الإرسال، تأكد من إعدادات الـ Secrets")
        else:
    ...
    try:
        # جلب البيانات مباشرة من الداتا بيز
